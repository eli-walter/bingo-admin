      ...
      {tabs.map(({ id, Icon, label }) => {
        ...
        return (
          <button key={id} onClick={() => setTab(id)}
            style={{ flex: 1, padding: "10px 0 14px", border: "none", background: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, position: "relative" }}>
...
function Dashboard({ ctx }) {
  ...
  return (
    <div className="slide-up" style={{ padding: "28px 16px 16px" }}>
      {/* Header */}
      <div style={{ marginBottom: 24 }}>
        <p style={{ fontSize: 12, color: C.muted, marginBottom: 3, fontWeight: 600 }}>ADMIN PANEL</p>
        <h1 style={{ fontFamily: "Syne,sans-serif", fontSize: 28, fontWeight: 900, color: C.text, lineHeight: 1.1 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[
          ...
          <div key={label} style={{ background: C.surface, borderRadius: 14, padding: "14px 12px", border: `1px solid ${C.border}` }}>
            <p style={{ fontSize: 9, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 6 }}>{label}</p>
            <p style={{ fontFamily: "Syne,sans-serif", fontSize: 18, fontWeight: 800, color, lineHeight: 1 }}>{value}</p>
            <p style={{ fontSize: 10, color: C.muted, marginTop: 3 }}>{sub}</p>
          </div>
        ))}
      </div>
...
export default function App() {
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", background: C.bg, minHeight: "100vh", fontFamily: "Nunito,sans-serif", position: "relative", color: C.text }}>
